...
var parseRawBody = function (request, reply, next) {
...
module.exports = function (plugin, options, next) {
  ...
  },
  {
    ...
    config: {
      ...
      handler: function (request, reply) {
        console.log('DB chosen >>>>>>>>>>>>>>>', request.pre.db);
    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        var opts = 'new_edits' in payload
          ...
          request.db.bulkDocs(payload, opts, function (err, response) {
            if (err) {
              return reply(err).code(err.status || 500);
            }
    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        for (var prop in request.body) {
          request.query[prop] = request.query[prop] || request.body[prop];
        }
...

  {
    ...
    config: {
      ...
      handler: function (request, reply) {
        request.pre.db.revsDiff(request.body || {}, function (err, diffs) {
          if (err) {
            return reply(err).code(400);
          }
...

  {
    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        if (body.map) {
          body.map = (new Function('return ' + body.map))();
        }
        request.query.conflicts = true;
        request.pre.db.query(body, query, function (err, response) {
          if (err) {
            return reply(err).code(400);
          }
          reply(response);
  ...
  // Put a document attachment
  {
    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        var type = request.get('Content-Type') || 'application/octet-stream';
    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        if (/^multipart\/related/.test(request.headers['content-type'])) {
          ...
          // normal PUT
          request.body._id = request.body._id || request.query.id;
          if (!request.body._id) {
            request.body._id = (!!request.params.id && request.params.id !== 'null')
              ? request.params.id
              : null;
          }
          request.pre.db.put(request.body, request.query, onResponse);
        }
      }
...

  {
    ...
    config: {
      ...
      handler: function (request, reply) {
        request.body._id = uuids(1)[0];
        request.pre.db.put(request.body, request.query, function (err, response) {
          if (err) {
            return reply(err).code(err.status || 500);
          }
          reply(response).code(201);
...

  {
    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        request.db.get(request.params.id, request.query, function (err, doc) {
          if (err) {
            return reply(err).code(404);
          }
